...
var queryParamsCollection, restClient;
 ...
 * @param cb
 */
function loadPart__(partIndex, paging, resultCollection, cb) {
    // queryParamsCollection и restClient - переменные уровня модуля
    if (queryParamsCollection[partIndex] && ('count' in paging ? paging.count !== 0 : true)) {
        var _params = _.extend({}, queryParamsCollection[partIndex], paging);
...
function loadPart (queryParams, collection, cb) {
    var restClient
}
...
var load = function () {
    ...
    // .. или query
        ...
        queryParamsCollection = query.getQueryParameters(this.options);

        var paging = {};
        if (queryParamsCollection[0].start) paging.start = queryParamsCollection[0].start;
        if (queryParamsCollection[0].count) paging.count = queryParamsCollection[0].count;

        var operations = [];
        for (var index = 0; index < queryParamsCollection.length; index++) {
            operations[index] = ;
